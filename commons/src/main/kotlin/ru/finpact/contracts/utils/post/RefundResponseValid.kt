...
class RefundResponseValid : Postcondition {
    ...
    override fun verify(ctx: ContractContext) {
        ...
        if (refundId <= 0L) throw ContractViolation.internal("result.refundPaymentId must be positive")

        val originalId = readLong(k, res, "originalPaymentId")
        if (originalId <= 0L) throw ContractViolation.internal("result.originalPaymentId must be positive")
