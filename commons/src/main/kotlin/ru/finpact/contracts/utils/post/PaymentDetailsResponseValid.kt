...
class PaymentDetailsResponseValid : Postcondition {
    ...
    override fun verify(ctx: ContractContext) {
        ...
        if (id <= 0L) throw ContractViolation("result.id must be positive")
        ...
        if (!isCurrencyUpper3(currency)) throw ContractViolation("result.currency must be 3-letter uppercase code")
        if (!Currency.isSupported(currency)) {
            val allowed = Currency.supportedCodes().joinToString(",")
            throw ContractViolation("result.currency '$currency' is not supported (allowed: $allowed)")
        }
        ...
        val fromActive = readBoolean(fromK, fromObj, "isActive")

        if (fromId <= 0L) throw ContractViolation("result.from.id must be positive")
        if (!isCurrencyUpper3(fromCurrency)) throw ContractViolation("result.from.currency must be 3-letter uppercase code")
        if (fromCurrency != currency) throw ContractViolation("result.from.currency must match result.currency")
        if (fromBalance < BigDecimal.ZERO) throw ContractViolation("result.from.balance must be >= 0")
        ...
        if (toId <= 0L) throw ContractViolation("result.to.id must be positive")
        if (toId == fromId) throw ContractViolation("result.from.id and result.to.id must be different")
        if (!isCurrencyUpper3(toCurrency)) throw ContractViolation("result.to.currency must be 3-letter uppercase code")
        if (toCurrency != currency) throw ContractViolation("result.to.currency must match result.currency")
    }
