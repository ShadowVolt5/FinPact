...
class TransferResponseValid : Postcondition {
    ...
    override fun verify(ctx: ContractContext) {
        ...
        if (fromId <= 0L) throw ContractViolation("result.fromAccountId must be positive")
        if (toId <= 0L) throw ContractViolation("result.toAccountId must be positive")
        if (fromId == toId) throw ContractViolation("result accounts must be different")
        ...
        if (amountTrimmed.isEmpty()) throw ContractViolation("result.amount must not be blank")
        if (!moneyRegex.matches(amountTrimmed)) throw ContractViolation("result.amount must be plain decimal with scale <= 4")

        val amount = try { BigDecimal(amountTrimmed) } catch (_: Throwable) {
            throw ContractViolation("result.amount must be a valid decimal number")
        }
        if (amount <= BigDecimal.ZERO) throw ContractViolation("result.amount must be positive")
        if (amount.scale() > 4) throw ContractViolation("result.amount scale must be <= 4")

        val integerDigits = max(0, amount.precision() - amount.scale())
        if (integerDigits > 15) throw ContractViolation("result.amount integer digits must be <= 15")

        val currencyRaw = (k.memberProperties.firstOrNull { it.name == "currency" }?.getter?.call(res) as? String)
            ?: throw ContractViolation("result.currency must be provided")
        ...
        val createdAtRaw = (k.memberProperties.firstOrNull { it.name == "createdAt" }?.getter?.call(res) as? String)
            ?: throw ContractViolation("result.createdAt must be provided")
