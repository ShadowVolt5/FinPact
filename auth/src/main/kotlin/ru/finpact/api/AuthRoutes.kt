...
fun Application.authRoutes() {
    ...
    val rawAuthService = AuthServiceImpl(
        authRepository = repo,
    )
    val authService: AuthService = ContractProxy.wrap<AuthService>(rawAuthService)

    val subjectPort = SubjectExistencePortImpl(repo)

    val rawTokenAuthService = TokenAuthServiceImpl(subjectPort)
    val tokenAuthService: TokenAuthService = ContractProxy.wrap<TokenAuthService>(rawTokenAuthService)

    routing {
        route("/auth") {
            post("/register") {
                val request = call.receive<RegisterRequest>()
                val result: RegisterResponse = authService.register(request)
                call.respond(HttpStatusCode.Created, result)
            }

            post("/login") {
                val request = call.receive<LoginRequest>()
                val result: LoginResponse = authService.login(request)
